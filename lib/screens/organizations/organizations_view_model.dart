...
abstract class OrganizationsViewModel extends State<Organizations> {
  ...
  Future<void> getOrganization(bool init) async {
    toggelIsLoading(true);
    try {
      dynamic organizationsResult =
          await Providers.getUserOrganization(user.login);
      print(organizationsResult);

      await store.dispatch(
          SetUserOrganizations(organizations: organizationsResult.data));
    } catch (e) {
      print(e.toString());
    } finally {
      toggelIsLoading(true);
